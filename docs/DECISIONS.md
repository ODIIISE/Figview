...
| D-011 | 2026-08-24 | AI: deferred integration, reserved seams. No agent API ships now, but from Phase A the architecture defines `fig-assist` traits — `ContextProvider` + `ScenarioRegistry` — so a **scenario-based contextual assistant** (canvas/layer/inspector/export/page each get their own assistant behavior) can attach later via API or local models without refactor. All edits flow through the command bus, making any agent "just another command producer." Local-first; cloud adapters opt-in and labeled when the time comes | User directive: connect an AI agent API later, save the spots now; assistant must be scenario-based per feature/page/layer | VISION P6, §5.1 | Accepted |
