...
pub fn get_color(obj: &serde_json::Value, name: &str) -> Option<Color> {
    let c = obj.get(name)?;
    Some(Color {
        r: c.get("r")?.as_f64()? as f32,
        g: c.get("g")?.as_f64()? as f32,
        b: c.get("b")?.as_f64()? as f32,
        a: c.get("a")?.as_f64().unwrap_or(1.0) as f32,
    })
}
...
pub fn get_matrix(obj: &serde_json::Value, name: &str) -> Option<Matrix> {
    let m = obj.get(name)?;
    Some(Matrix {
        m00: m.get("m00")?.as_f64()? as f32,
        m01: m.get("m01")?.as_f64()? as f32,
        m02: m.get("m02")?.as_f64()? as f32,
        m10: m.get("m10")?.as_f64()? as f32,
        m11: m.get("m11")?.as_f64()? as f32,
        m12: m.get("m12")?.as_f64()? as f32,
    })
}
...
pub fn get_parent_index(obj: &serde_json::Value) -> Option<(NodeId, String)> {
    ...
    Some((
        NodeId::new(
            guid.get("sessionID")?.as_u64()? as u32,
            guid.get("localID")?.as_u64()? as u32,
        ),
        pos,
    ))
}
