...
fn serialization_matches_frontend() {
    ...
    assert_eq!(etype, "DropShadow", "frontend expects 'DROP_SHADOW' but Rust sends {:?}", etype);
    ...
    let node = FigNode {
        ...
        size: None, transform: None, corner_radius: None,
        fill_paints: vec![], stroke_paints: vec![],
    ...
    let node2 = FigNode { phase: Some(NodePhase::Removed), ..node };
